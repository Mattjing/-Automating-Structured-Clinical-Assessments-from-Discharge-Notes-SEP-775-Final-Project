...
class TestMDSMapperBasic:
    ...
    def test_section_n_integer_field_mapped(self, mapper):
        extraction = {"N0400E": 5, "confidence": {"N0400E": 0.8}}
        a = mapper.map("n1", "p1", "h1", extraction)
        assert a.get_field("N0400E") == 5

    def test_section_o_boolean_field_mapped(self, mapper):
        extraction = {"O0100C1": True, "confidence": {"O0100C1": 0.9}}
        a = mapper.map("n1", "p1", "h1", extraction)
        assert a.get_field("O0100C1") is True
